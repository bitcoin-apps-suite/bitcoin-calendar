...
export const getThemedIcon = (iconId: string, theme: string = 'lucide') => {
  ...
  if (!mapping) {
    ...
    const defaultMapping = iconMappings['settings'] || { lucide: Settings, 'react-icons': MdSettings }
    return defaultMapping[theme as keyof IconMapping] || defaultMapping.lucide
  }
  
  return mapping[theme as keyof IconMapping] || mapping.lucide
}

export const getCurrentTheme = (): string => {
  if (typeof window === 'undefined') return 'lucide'
  const theme = localStorage.getItem('bitcoinOS-icon-theme') || 'lucide'
  // Support basic themes
  const validThemes = ['lucide', 'react-icons']
